...
class Ultrasonic extends EventEmitter {
  ...
  search () {
    ...
    if (this.lastDeltaSentInterval === null) {
      this.lastDeltaSentInterval = setInterval(() => {
        ...
        const statusData = `Last data received ${last}`
        debug(statusData)

        this.emit('status', {
          status: 'last_data_received',
          data: statusData
        })
      }, 5000)
    }
  ...
  decode (buf) {
    ...
    // TODO: in my testing the wind angles don't always end up right; may be a problem of the Dyson fan. Must test on boat. Theory is correct.
